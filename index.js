...
const SYSTEM_ROA_JSON = "You are Riya, a South African FAIS compliance assistant. Generate a complete FAIS-compliant Record of Advice as a JSON object only. No preamble, no markdown, no backticks - only the raw JSON object. The JSON must have exactly this structure: {\"section1\":{\"title\":\"FSP AND REPRESENTATIVE DETAILS\",\"fields\":[{\"label\":\"Financial Services Provider\",\"value\":\"...\"},{\"label\":\"FSP Registration Number\",\"value\":\"...\"},{\"label\":\"Representative Name\",\"value\":\"...\"},{\"label\":\"Professional Indemnity Insurance\",\"value\":\"...\"},{\"label\":\"Date of Advice\",\"value\":\"...\"},{\"label\":\"Advice Type\",\"value\":\"...\"},{\"label\":\"Communication Method\",\"value\":\"...\"}]},\"section2\":{\"title\":\"CLIENT IDENTIFICATION, KYC, FICA AND POPIA\",\"fields\":[{\"label\":\"Client Name\",\"value\":\"...\"},{\"label\":\"Client Type\",\"value\":\"...\"},{\"label\":\"Identity Verification\",\"value\":\"...\"},{\"label\":\"Proof of Address\",\"value\":\"...\"},{\"label\":\"POPIA Consent\",\"value\":\"...\"},{\"label\":\"FICA Compliance\",\"value\":\"...\"},{\"label\":\"Claims History\",\"value\":\"...\"},{\"label\":\"FAIS Disclosure\",\"value\":\"...\"}]},\"section3\":{\"title\":\"NEEDS ANALYSIS\",\"paragraphs\":[\"paragraph 1\",\"paragraph 2\"]},\"section4\":{\"title\":\"MARKET COMPARISON\",\"paragraphs\":[\"paragraph 1\"]},\"section5\":{\"title\":\"PRODUCT RECOMMENDED\",\"fields\":[{\"label\":\"Insurer\",\"value\":\"...\"},{\"label\":\"Policy Number\",\"value\":\"...\"},{\"label\":\"Premium\",\"value\":\"...\"},{\"label\":\"Cover Basis\",\"value\":\"...\"},{\"label\":\"Effective Date\",\"value\":\"...\"}],\"paragraphs\":[\"cover details paragraph\",\"exclusions paragraph\",\"SASRIA paragraph\",\"excess structure paragraph\",\"tracking requirements paragraph\"]},\"section6\":{\"title\":\"REMUNERATION AND CONFLICT OF INTEREST\",\"paragraphs\":[\"paragraph 1\"]},\"section7\":{\"title\":\"REPLACEMENT ADVICE\",\"paragraphs\":[\"paragraph 1\"]},\"section8\":{\"title\":\"CLIENT ACCEPTANCE RECORD\",\"fields\":[{\"label\":\"Method of Acceptance\",\"value\":\"...\"},{\"label\":\"Date\",\"value\":\"...\"}],\"paragraphs\":[],\"signature_block\":true}}. CRITICAL: never use section numbers above 8. Use exact insurer names from input. Output ONLY the JSON object.";function isHcvGitContent(text) {
  const t = text.toLowerCase();
  const keywords = ['truck', 'trailer', 'fleet', 'hcv', 'heavy commercial', 'goods in transit', 'git', 'per conveyance', 'haulage', 'transport company', 'driver fidelity', 'deposit premium', '70/30', '50/50', 'load limit', 'icab', 'mix telematics', 'rms', 'prdp'];
  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const raw of rawLines) {
       const _prot = raw.replace(/\bPMVs\b/g,'__PMVS__').replace(/\bLDVs\b/g,'__LDVS__').replace(/\bMiX\b/g,'__MIX__').replace(/\bGoSure\b/g,'__GS__').replace(/\bRMS\b/g,'__RMS__').replace(/\bICAB\b/g,'__ICAB__').replace(/\bSASRIA\b/g,'__SASRIA__').replace(/\bONE\b/g,'__ONE__');
_prot.replace(/([a-z.,%)\d])([A-Z][a-z])/g,'$1\n$2').split('\n').map(s=>s.trim().replace(/__PMVS__/g,'PMVs').replace(/__LDVS__/g,'LDVs').replace(/__MIX__/g,'MiX').replace(/__GS__/g,'GoSure').replace(/__RMS__/g,'RMS').replace(/__ICAB__/g,'ICAB').replace(/__SASRIA__/g,'SASRIA').replace(/__ONE__/g,'ONE')).filter(s=>s.length>0).forEach(s=>{if(s.trim())lines.push(s.trim());}); 
      }
