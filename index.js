...
CRITICAL ACCURACY RULES - NEVER INFER, ONLY REFLECT:
1. GENDER: Derive the client title strictly from the SA ID number. Digits 7-10 of a 13-digit SA ID: if the value is 0000-4999 the client is female - use Ms or Mrs; if 5000-9999 the client is male - use Mr. Never guess gender from a first name. If no ID number is present, use the full name only with no title.
2. VEHICLE REGISTRATION: If a registration number appears anywhere in the input, write it exactly as provided into the vehicle registration field. Never use a placeholder such as "As declared by client" if the actual registration number was supplied.
3. VEHICLE USE - NEVER INFER COMMERCIAL USE: The vehicle use classification must be taken exclusively from the declared Primary Use field. Occupation, employer type, or self-employment status must never override the declared use. If the input states Private use, the Needs Analysis and Product Classification must reflect Private use - never describe the vehicle as used for deliveries or commercial purposes unless the broker explicitly stated this.
4. COMMUNICATION METHOD: Record all communication methods stated in the input. If both a face-to-face meeting and an email confirmation are mentioned, record both. Never reduce multiple stated methods to a single one.
5. ONLY PRODUCE WHAT IS IN THE INPUT: Do not infer, assume, or add any fact, classification, or description not explicitly stated by the broker.
";
...
function buildSystemRoa(userText, language) {  const base = language === 'af' ? SYSTEM_ROA_AF : SYSTEM_ROA;
  ...
  if (isAgriContent(userText)) return base + agri;
  return base;
}
...
KRITIEKE AKKURAATHEIDSREELS - MOENIE AFLEI NIE, WEERSPIEËL SLEGS:
1. GESLAG: Lei die titel streng af uit die SA ID-nommer. Syfers 7-10 van 'n 13-syfer SA ID: as die waarde 0000-4999 is, is die klient vroulik - gebruik Mev. of Me.; as dit 5000-9999 is, is die klient manlik - gebruik Mnr. Moenie geslag uit 'n voornaam raai nie. As geen ID beskikbaar is nie, gebruik die volledige naam sonder titel.
2. VOERTUIGREGISTRASIENOMMER: As 'n registrasienommer in die invoer verskyn, skryf dit presies soos verskaf. Gebruik nooit 'n plekhouer soos "Soos verklaard deur klient" as die werklike nommer verskaf is nie.
3. VOERTUIGGEBRUIK - MOENIE KOMMERSIËLE GEBRUIK AFLEI NIE: Die gebruikklassifikasie moet uitsluitlik geneem word uit die verklaarde Primere Gebruik-veld. Beroep of selfstandige status moet nooit die verklaarde gebruik oorskry nie. As die invoer Privaat gebruik vermeld, moet die Behoefteanalise en Produkklassifikasie Privaat gebruik weerspieël - beskryf die voertuig nooit as gebruik vir aflewerings of kommersiële doeleindes nie, tensy die makelaar dit uitdruklik gestel het.
4. KOMMUNIKASIEMETODE: Teken alle kommunikasiemetodes aan wat in die invoer vermeld word. Moenie verskeie metodes tot een verminder nie.
5. PRODUSEER SLEGS WAT IN DIE INVOER IS: Moenie enige feit of klassifikasie aflei of byvoeg wat nie uitdruklik deur die makelaar gestel is nie.
";
