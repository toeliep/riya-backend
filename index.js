  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const raw of rawLines) {
        raw.split(/(?<=[a-z.,%])(?=[A-Z])/).forEach(s => { if (s.trim()) lines.push(s.trim()); });
      }
