...
// Ensures every numbered section heading (e.g. "3. NEEDS ANALYSIS") starts on
// its own line, even if the model ran it directly onto the end of the prior
// sentence with no line break. Matches a digit(s)+period+space+3-or-more
// consecutive uppercase words, which is the section-heading pattern used
// throughout the RoA prompt.
function forceHeadingLinebreaks(text) {
  let out = text.replace(/([^\n])(\d{1,2}\.\s+[A-Z][A-Z\s,&/-]{4,})/g, '$1\n\n$2');
  // Also split where an ALL-CAPS heading phrase runs directly into a
  // Title-Case word with no space (e.g. "NEEDS ANALYSISMotor Vehicle").
  out = out.replace(/\b([A-Z]{2,}(?:[\s,&/-]+[A-Z]{2,})*)([A-Z][a-z])/g, '$1\n$2');
  return out;
}
 
// Given a part of the RoA expected to start at a specific section number,
// strips any preamble (a stray title, or a restatement of earlier sections)
// that the model may have generated before actually reaching that section's
// heading. This makes the output correct regardless of whether the model
// followed the "begin directly with section N" instruction. If the expected
// heading isn't found at all, returns the text unchanged (fail-safe).
...
// Defensive fallback: guarantee a client + adviser signature block always
// exists, even if the model didn't include one in Section 8.
