...
function buildSystemRoa(userText) {
  if (isAgriContent(userText)) return SYSTEM_ROA + AGRI_GUIDANCE;
  return SYSTEM_ROA;
}
...
app.post('/generate-roa', async (req, res) => {
  ...
  try {
    ...
    let part1 = '';
    const activeSystemRoa = buildSystemRoa(user);
