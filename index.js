...
app.post('/generate-pdf', async (req, res) => {
  ...
  try {
    ...
    // heading (e.g. "DETAILSFinancial...") is split before parsing sections.
        const cleaned = text.replace(/Generated by Riya[^\n]*/gi, '').replace(/Produced by Riya[^\n]*/gi, '').replace(/Riya FAIS Compliance[^\n]*/gi, '').replace(/Riya \| FAIS[^\n]*/gi, '').trim();
    ...
    for (const section of sections) {
      ...
      for (const raw of rawLines) {
        raw.split(/(?<=[a-z.,%\d\)\]])(?=[A-Z])|(?<=[A-Z]{2,})(?=[A-Z][a-z])|(?<=:)(?=\d)/).forEach(s => { if (s.trim()) lines.push(s.t  
      }
